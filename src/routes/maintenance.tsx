...
export const Route = createFileRoute("/maintenance")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Manage preventive maintenance, warranty repairs, and vendor tickets." },
      { property: "og:title", content: "Maintenance — AssetFlow" },
      { property: "og:description", content: "Manage preventive maintenance, warranty repairs, and vendor tickets." },
    ],
  }),
...
const statusOptions: MaintenanceStatus[] = [
  "Scheduled", "In Progress", "Awaiting Parts", "Completed", "Cancelled",
];

function statusTone(s: MaintenanceStatus) {
  return s === "In Progress" ? "info"
    : s === "Completed" ? "success"
    : s === "Awaiting Parts" ? "warning"
    : s === "Cancelled" ? "muted" : "primary";
}
...
function priorityDot(p: MaintenancePriority) {
  ...
  return (
    <span className="inline-flex items-center gap-1.5 text-sm text-foreground">
      <span className={cn("h-2 w-2 rounded-full", map[p])} />
      {p}
    </span>
  );
}
  ...
  const filtered = useMemo(
    () =>
      maintenanceTickets.filter((t) => {
        const matchQ =
          q.trim() === "" ||
          [t.ticketNo, t.assetTag, t.assetName, t.summary, t.vendor, t.assignedTech]
    ...
    >
      ...
      <div className="flex flex-col gap-3 rounded-lg border border-border bg-card p-4 shadow-sm sm:flex-row sm:items-center">
        <div className="relative flex-1">
          ...
          <Input
            placeholder="Search by ticket, asset, vendor, or technician..."
            value={q}
              ...
              <TableRow key={t.id}>
                ...
                <TableCell className="text-sm text-muted-foreground">
                  <span className="inline-flex items-center gap-1.5">
                    <Wrench className="h-3.5 w-3.5" /> {t.type}
                  </span>
                </TableCell>
