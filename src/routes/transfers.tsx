...
export const Route = createFileRoute("/transfers")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Track inter-office asset movement, shipments, and approval workflow." },
      { property: "og:title", content: "Transfers — AssetFlow" },
      { property: "og:description", content: "Track inter-office asset movement, shipments, and approval workflow." },
    ],
  }),
  ...
  const filtered = useMemo(
    () =>
      transfers.filter((t) => {
        const matchQ =
          q.trim() === "" ||
          [t.transferNo, t.assetTag, t.assetName, t.fromLocation, t.toLocation]
    ...
    >
      ...
      <div className="flex flex-col gap-3 rounded-lg border border-border bg-card p-4 shadow-sm sm:flex-row sm:items-center">
        <div className="relative flex-1">
          ...
          <Input
            placeholder="Search by transfer no., asset, or location..."
            value={q}
              ...
              <TableRow key={t.id}>
                ...
                <TableCell className="text-sm text-muted-foreground">
                  <span className="inline-flex items-center gap-1.5">
                    <Truck className="h-3.5 w-3.5" /> {t.carrier}
                  </span>
                </TableCell>
